...
const userConnect = (user_id, socket_id, socket) => {
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      let messages = await DBAccess.message.getUndelievredMessagesOfusser(
        user_id
      ); // GET UN DELIVERED MESSAGE
      let message_entity;
      if (messages.length > 0) {
        ...
        for (let i = 0; i < messages.length; i++) {
          ...
          if (reciever.length > 0 && reciever[0].socket_id) {
            ...
            if (sender.length > 0 && sender[0].socket_id) {
              ...
              socket
                .to(sender[0].socket_id)
                .emit("message_delivered", messages[i].chatid);
        ...
        let messagesEmit =
          await DBAccess.message.getUndelievredMessageEmitOfusser(user_id);
        if (messagesEmit.length > 0) {
          console.log("UN DELIVERED MESSAGE EMIT : ", messagesEmit.length);
          for (let i = 0; i < messagesEmit.length; i++) {
            let sender = await DBAccess.user.getUserById(
              messagesEmit[i].sender
            );
            if (sender.length > 0 && sender[0].socket_id) {
              console.log("MESSAGE DELIVERED 2 : ", messagesEmit[i].chatid);
              socket
                .to(sender[0].socket_id)
                .emit("message_delivered", messagesEmit[i].chatid);
              await DBAccess.message.updateEmitStatus(
                messagesEmit[i].chatid,
                2
              ); // UPDATE DELIVERY EMIT STATUS
            }
          }
        }
      }
